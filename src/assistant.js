...
async function parseNoteFromText(userText) {
  const prompt =
    ...
    `輸入文字：${userText}\n\n` +
    '【要求】所有欄位皆須為完整且語意通順之繁體中文，嚴禁在標題或詳細說明中使用省略號「...」或未完結句子。\n\n' +
    '請輸出繁體中文 JSON：\n' +
    '{\n' +
    '  "category": "分類標籤 (例如: 📅 日程會議 / 📋 待辦交辦 / 💰 報價成本 / 📐 工程技術 / 💡 備忘靈感 / 📜 法規送審)",\n' +
    '  "title": "簡潔精準且完整的記事標題",\n' +
    '  "details": "詳細內容或補充說明 (無則可留空)",\n' +
  ...
  try {
    ...
    return {
      category: '📋 待辦事項',
      title: userText.replace(/^(記下|幫我記|記事|備忘|提醒我|待辦|todo|note)[:：\s]*/i, '').trim() || userText,
      details: '',
      dueDate: '未指定',
    };
  }
...
async function synthesizeAll({ messages = [], notes = [], displayName = '使用者' }) {
  ...
  const systemPrompt =
    ...
    '【撰寫原則】所有欄位皆須為完整且語意通順之專業語句，嚴禁在任何欄位中使用省略號「...」或未完結句子。\n\n' +
    '請輸出符合下列繁體中文 JSON 格式：\n' +
    '{\n' +
    '  "overview": "今日工作與對話推進總結",\n' +
    '  "coreDecisions": ["重要決策/共識1", "重要決策/共識2"],\n' +
    '  "actionItems": ["待辦與工程交辦事項1 (註明負責人或期限)", "待辦事項2"],\n' +
    '  "keyData": ["重要數據、建材報價或法規指標備忘1", "關鍵數據2"],\n' +
    '  "risksAndWatch": "實務工程介面、法規或時程之潛在風險預警",\n' +
    '  "strategicAdvice": "總監級下一步推進戰略建議"\n' +
    '}\n' +
    '請只輸出純 JSON 格式字串，不要加 markdown 標記。';
...
async function analyzeNewsDeeply(newsTitle) {
  const prompt =
    ...
    `請針對以下新聞主題進行深度產業洞察剖析：\n\n` +
    `新聞主題：${newsTitle}\n\n` +
    '【撰寫要求】請深入拆解技術、法規、市場與策略，所有欄位皆須為完整專業之繁體中文語句，嚴禁在任何欄位中使用省略號「...」或截斷句子！\n\n' +
    '請輸出符合下列繁體中文 JSON 格式：\n' +
    '{\n' +
    '  "category": "主題分類標籤 (例如: 🌿 綠建ESG / 📜 政策法規 / 🏗️ 重大工程 / 🏢 智慧科技 / 🏛️ 空間設計)",\n' +
    '  "context": "事件核心脈絡與背景成因",\n' +
    '  "techImpact": "工程技術、施工工法或建築設計面之衝擊與要求",\n' +
    '  "policyImpact": "法規政策、都更獎勵或制度規範之影響評估",\n' +
    '  "marketOpportunity": "產業供應鏈、市場趨勢與商機效益分析",\n' +
    '  "strategyAdvice": "給建築師、營造廠或開發商的專家行動策略建議"\n' +
    '}\n' +
    '請只輸出純 JSON 格式字串，不要加 markdown 標記。';
